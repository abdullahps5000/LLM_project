...
class DistributedInferenceEngine:
    ...
    def generate(
        ...
        try:
            ...
            # Validate logits shape: should be [batch, seq_len, vocab_size]
            if len(logits.shape) != 3:
                raise RuntimeError(f"Expected logits shape [batch, seq_len, vocab_size], got {logits.shape}")
            
            batch_size, seq_len, vocab_size = logits.shape
            logger.debug(f"Logits: batch={batch_size}, seq_len={seq_len}, vocab_size={vocab_size}")
            
            # Check if vocab_size is reasonable (should be > 1000 for most models)
            if vocab_size < 1000:
                logger.warning(f"Suspicious vocab_size: {vocab_size}. Expected > 1000 for most models.")
